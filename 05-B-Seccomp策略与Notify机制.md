...
```
parse_config(conf)
  1. 读取策略文件第一行 → 确定版本号（1 或 2）
  2. v1: parse_config_v1()
  3. v2:
     a. 读取 allowlist/denylist 声明
     b. 设置默认 action：
        - allowlist → 默认 SCMP_ACT_KILL（拒绝未列出的）
        - denylist → 默认 SCMP_ACT_ALLOW（允许未列出的）
     c. 为每个需要的架构创建 filter context
     d. 逐行读取规则：
        - 解析架构标签 [xxx]
        - 解析 "syscall_name action [args]"
        - 调用 seccomp_rule_add() 添加到对应 ctx
     e. 合并所有架构的 ctx（seccomp_merge）
```
...
```
┌─────────────────┐      ┌──────────────────┐      ┌──────────────┐
│  容器进程         │      │  LXC Supervisor   │      │  Notify Proxy │
│                 │      │                  │      │  (可选外部)    │
│  syscall()      │──①──→│  notify handler   │──③──→│  处理请求     │
│    ↑            │      │    ↑              │      │    ↓          │
│    └──────⑤─────│←──④──│    └───②──────────│←─────│  返回应答     │
│                 │      │                  │      │              │
└─────────────────┘      └──────────────────┘      └──────────────┘

① 系统调用触发 SCMP_ACT_NOTIFY
② seccomp_notify_receive() 接收请求
③ 转发给外部 proxy（如果配置）
④ 接收 proxy 应答
⑤ seccomp_notify_respond() 返回结果给容器进程
```
...
seccomp_notify_setup_proxy()
  ...
  ├── 连接到 lxc.seccomp.notify.proxy 指定的地址
  ├── 分配 notify 缓冲区
  └── 注册到 mainloop 事件循环
```
...
容器端:
  ...
  // 通过 Unix socket 将 notify fd 发送给 supervisor
...
seccomp_notify_handler(fd, mainloop)
  ├── seccomp_notify_receive(fd, &req)        // 接收系统调用请求
  │     // req 包含：syscall number, args, pid, id
  │
  ├── 如果有 proxy 连接：
  │     ├── seccomp_notify_id_valid(fd, req.id)  // 验证请求仍有效
  │     │     // 进程可能已退出
  │     │
  │     ├── 发送请求到 proxy                      // 通过 Unix socket
  │     │     // 包含 req + cookie + 附加 fd
  │     │
  │     ├── 接收 proxy 应答                       // resp + flags
  │     │
  │     └── seccomp_notify_respond(fd, &resp)    // 返回给内核
  │           // resp 包含：error, val, flags
  │
  └── 无 proxy 时：
        └── 返回默认应答 -ENOSYS                  // 拒绝但不杀进程
```
