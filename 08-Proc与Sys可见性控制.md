...
lxc_mount_auto_mounts(handler, flags)
  1. Prepare /proc mountpoint (518-539)
     +- check whether /proc already exists
     +- create directory if needed
     `- ensure mountpoint is usable

  2. Prepare /sys mountpoint (541-562)
     `- same as above

  3. Walk default_mounts[] table (565-616)
     for entry in default_mounts[]:
         if (entry.flags & requested_flags):
             replace %r placeholder
             run mount operation

  4. Mount cgroup (618-648)
     +- choose default perms by CAP_SYS_ADMIN
     `- call handler->cgroup_ops->mount()

  5. Shared-mount helpers (650-653)
```
...
lxc_setup() flow:
  ...
  1. lxc_rootfs_prepare_child()           // prepare rootfs
  2. lxc_setup_rootfs_prepare_root()      // set root filesystem (3774-3776)
  3. mount_autodev()                      // mount /dev tmpfs (3800-3804)

  4. * First automount: proc + sys        // (3806-3811)
     lxc_mount_auto_mounts(flags & ~CGROUP)
     // mount proc and sys only, not cgroup
     // because cgroup may depend on /sys

  5. setup_mount_fstab()                  // handle lxc.mount.fstab (3813-3815)
  6. setup_mount_entries()                // handle lxc.mount.entry (3817-3820)
  7. handle idmapped mounts (3823-3828)

  8. * Second automount: cgroup only      // (3835-3841)
     lxc_mount_auto_mounts(flags & CGROUP)
     // /sys already exists, safe to mount /sys/fs/cgroup

  9. run mount hooks (3843-3848)
  10. lxc_setup_boot_id()                 // replace boot_id (3850-3852)
  11. lxc_setup_devpts()                  // set up devpts
  12. lxc_setup_rootfs_switch_root()      // pivot_root (3887-3891)
  13. setup_sysctl_parameters()           // write sysctl (3906-3912)
```
...
```
/proc                       <- proc filesystem, fully rw
+- sys/                     <- writable (can change kernel params!)
|  +- net/                  <- writable
|  `- kernel/               <- writable
+- sysrq-trigger            <- writable (danger!)
+- kcore                    <- readable (but limited by AppArmor)
`- ...
```
...
+- sys/                     <- read-only (bind + remount ro)
|  `- net/                  <- writable (moved back to rw mount)
+- sysrq-trigger            <- read-only (bind + remount ro)
`- rest                     <- normal rw
```
...
```
Initial state: /proc/sys/net (rw)

Step 1: bind mount /proc/sys/net -> /proc/tty      (save net)
Step 2: bind mount /proc/sys -> /proc/sys           (prepare remount)
Step 3: remount /proc/sys -> readonly               (make full sys read-only)
Step 4: bind mount /proc/tty -> /proc/sys/net       (restore net rw)

Result: /proc/sys (ro), /proc/sys/net (rw)
```
...
```c
// Each container gets a unique boot_id
// Prevent reading the host boot_id

1. generate random UUID -> write /dev/.lxc-boot-id
...
```
/sys                                <- sysfs, read-only
+- devices/virtual/net/             <- writable (bind mount from temp sysfs)
|  +- eth0/                         <- writable
|  `- lo/                           <- writable
`- rest                             <- read-only
```
...
```
Step 1: mount sysfs (rw) -> /proc/sys           (temp mountpoint)
Step 2: mount sysfs (ro) -> /sys                (read-only target)
Step 3: bind /proc/sys/devices/virtual/net
         -> /sys/devices/virtual/net            (writable net devices)
Step 4: umount /proc/sys                        (clean temp mount)
```
...
lxc_pivot_root(rootfs)
  1. mount("", "/", NULL, MS_REC|MS_PRIVATE, NULL)
     // make mount propagation private to avoid leaks
  2. mount(rootfs, rootfs, NULL, MS_BIND|MS_REC, NULL)
     // bind-mount rootfs onto itself
  3. chdir(rootfs)
  4. pivot_root(".", ".")
     // new root = rootfs, old root = rootfs (then umount)
  5. umount2(".", MNT_DETACH)
     // unmount old root filesystem
  6. chdir("/")
```
...
lxc_chroot(rootfs)
  ...
  2. chroot(rootfs)
  3. chdir("/")
  4. unmount old mountpoints one by one
```
...
Container start
  ...
  |    `- set rootfs mount (bind/loop/...)
  |
  +- mount_autodev()
  |    `- /dev <- tmpfs + device nodes + symlinks
  |
  +- * first lxc_mount_auto_mounts()
  |    +- /proc <- proc filesystem
  ...
  |    `- user-defined mounts (lxc.mount.fstab / lxc.mount.entry)
  |
  +- * second lxc_mount_auto_mounts()
  |    `- /sys/fs/cgroup <- cgroup filesystem
  |
  +- lxc_setup_boot_id()
  |    `- /proc/sys/kernel/random/boot_id <- container UUID (ro)
  |
  +- lxc_setup_rootfs_switch_root()
  |    `- pivot_root / chroot
  |
  `- setup_sysctl_parameters()
       `- write container sysctl params (now inside new rootfs)
```
