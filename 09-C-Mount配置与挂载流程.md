...
```
1. lxc_mount_auto_mounts(non-cgroup part) - proc, sysfs
2. setup_mount_fstab()                  - external fstab file
3. setup_mount_entries()                - inline mount entries
4. lxc_mount_auto_mounts(cgroup part)   - cgroup filesystem
```
...
set_config_mount()                              [confile.c:2426-2452]
  +-- append mount entry string to conf->mount_entries list
  `-- use add_elem_to_mount_list() to create string_entry node
```
...
setup_mount_entries()                           [conf.c:2555-2565]
  +-- write mount_entries list to a temporary memfd file
  +-- setmntent(memfd) opens it as an fstab stream
  `-- mount_file_entries(rootfs, mntent_stream, ...)
      `-- read getmntent_r() entries one by one
...
```
mount_entry()
  +-- parse LXC attrs (create/optional/relative/idmap)
  +-- parse VFS flags and propagation flags
  +-- choose mount location:
  |   +-- abs path with rootfs prefix -> mount_entry_on_absolute_rootfs()
  |   +-- target is a system path (/proc, /sys, etc.) -> mount_entry_on_systemfs()
  |   `-- otherwise -> mount_entry_on_relative_rootfs()
  `-- finally call mount_entry_on_generic()
```
...
mount_entry_on_generic()
  +-- mount_entry_create_dir_file()     // handle create=dir/file
  ...
  |       // bind mounts need a second mount to apply ro, etc.
...
setup_mount_fstab(rootfs, fstab_path, ...)      [conf.c:2462-2480]
  +-- setmntent(fstab_path, "r")               // open fstab file
  `-- mount_file_entries(rootfs, stream, ...)   // same path as mount.entry
```
...
2. mount("/proc/sys", "/proc/sys", NULL, MS_BIND, NULL)           // bind mount
3. mount("/proc/sys", "/proc/sys", NULL, MS_REMOUNT|MS_RDONLY|MS_BIND, NULL)  // remount read-only
4. mount("/proc/sys/net", "/proc/tty", "none", MS_BIND, NULL)     // stash net at temp point
5. umount("/proc/sys")                                             // unmount read-only /proc/sys
6. mount("/proc/tty", "/proc/sys/net", "none", MS_MOVE, NULL)      // move net back (keep writable)
```
...
2. create temporary sysfs mount
3. bind mount /sys/devices/virtual/net (writable)
4. remove temporary mount
```
...
lxc_setup()
  |
  +--1-- lxc_mount_auto_mounts(non-cgroup)
  |      +-- proc -> /proc (mode selected by auto_mounts flags)
  |      +-- sysfs -> /sys
  |      +-- /proc/sys read-only bind (mixed mode)
  |      +-- /proc/sysrq-trigger read-only bind
  |      `-- shmounts setup
  |
  +--2-- setup_mount_fstab(conf->fstab)
  |      `-- read external fstab and call mount_entry() for each line
  |
  +--3-- setup_mount_entries(conf->mount_entries)
  |      `-- walk list and call mount_entry() for each line
  |          +-- parse_lxc_mount_attrs()       // parse LXC extra options
  |          +-- parse_vfs_attr()              // parse VFS flags
  |          +-- mount_entry_create_dir_file() // create mountpoint
  |          +-- safe_mount()                  // perform mount
  |          +-- remount (bind mount + ro)     // two-step operation
  |          `-- set propagation flags
  |
  +--4-- lxc_mount_auto_mounts(cgroup)
  |      `-- cgroup_ops->mount(cgroup_ops, handler)
  |
  `--5-- hook.mount hook (if configured)
```
