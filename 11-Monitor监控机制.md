...
| lxc handler | -------------> | lxc-monitord | -----------------------> | lxc-monitor |
| (host-side  | write lxc_msg  | (host-side   | broadcast to subscribers | (host-side  |
|  parent)    |                |  daemon)     |                          |  client)    |
+-------------+                +--------------+                          +-------------+
```

| 组件 | 角色 | 源文件 |
|------|------|--------|
| lxc handler（宿主父进程） | 状态变化时写入 FIFO | `src/lxc/monitor.c` |
| lxc-monitord（宿主守护进程） | 转发守护进程（epoll 事件循环） | `src/lxc/cmd/lxc_monitord.c` |
| lxc-monitor（宿主客户端） | 用户空间命令行订阅工具 | `src/lxc/tools/lxc_monitor.c` |

> **重要澄清**：整个 monitor 通信完全发生在宿主端（host-side）。容器内的进程（如 PID 1 的 init）**不参与** monitor 通信。状态通知由宿主端的 handler 父进程产生——它通过同步协议和 `waitpid()` 感知容器状态变化，然后调用 `lxc_monitor_send_state()` 写入宿主文件系统上的 FIFO。

### 1.1 为什么容器内不参与

- FIFO 路径 (`$rundir/lxc/$lxcpath/monitor-fifo`) 在宿主文件系统上，容器的 mount namespace 看不到
- `lxc_set_state()` 的所有调用点都在宿主端 handler 进程中：
  - `start.c:974` — STARTING（在 `lxc_init()` 中）
  - `start.c:2153` — RUNNING（在 `lxc_spawn()` clone 后）
  - `start.c:1114` — STOPPING（在 `lxc_end()` 中）
  - `start.c:1169` — STOPPED（在 `lxc_end()` 中）
  - `start.c:2337` — exit_code（在 `__lxc_start()` 末尾 `waitpid()` 之后）
- 容器内进程只运行用户指定的 init 程序，完全不知道 monitor 的存在
